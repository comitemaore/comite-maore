...
class AdherentController extends AbstractController
{
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($nin !== '') {
                $existant = $this->connection->fetchAssociative(
                    'SELECT id_adht, nom_adht, prenom_adht, NIN_adh FROM comitemaore_adherent WHERE NIN_adh = ?',
                    [$nin]
                );
                if ($existant) {
                    $error = sprintf(
                        ...
                        $existant['id_adht'],
                        $this->generateUrl('app_adherent_show', ['id' => $existant['id_adht']])
                    );
                    // Réafficher le formulaire avec l'erreur et les données saisies
            ...
            if ($bypassActif) {
                try {
                    ...
                    $this->log('ajout_adherent_direct', 'succes', $idAdht, [
                        'bypass' => true,
                        'sections_completes' => $statut['completes'],
                    ]);
                    $this->addFlash('success',
                        "Adhérent ajouté directement (#$idAdht). "
                        . "Mode bypass actif ({$statut['completes']}/20 sections configurées)."
                    );
                    return $this->redirectToRoute('app_adherent_show', ['id' => $idAdht]);

                } catch (\Doctrine\DBAL\Exception\UniqueConstraintViolationException $e) {
                    // Filet de sécurité si le NIN a été inséré entre la vérification et l'insert
                    $existant = $this->connection->fetchAssociative(
                        'SELECT id_adht, nom_adht, prenom_adht FROM comitemaore_adherent WHERE NIN_adh = ?',
                        [$nin]
                    );
                    $lien = $existant
                        ? sprintf(' Voir la fiche existante : <a href="%s" style="color:inherit;text-decoration:underline;">#%d %s %s</a>.',
                            $this->generateUrl('app_adherent_show', ['id' => $existant['id_adht']]),
                            $existant['id_adht'],
                            htmlspecialchars($existant['prenom_adht']),
                            htmlspecialchars($existant['nom_adht']))
                        : '';
                    $error = 'Ce NIN est déjà utilisé par un autre adhérent.' . $lien
                           . ' Vérifiez lequel des deux enregistrements est incorrect.';
                }
            } else {
                // ---- WORKFLOW NORMAL : demande d'approbation ----
                ...
                $this->addFlash('success',
                    "Demande d'ajout créée (#$idApprobation). En attente d'approbations."
                );
    ...
    // ------------------------------------------------------------------
    // MODIFIER (toujours via workflow)
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // SUPPRIMER (toujours via workflow)
    // ------------------------------------------------------------------
